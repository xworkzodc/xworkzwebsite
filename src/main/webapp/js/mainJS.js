...
var testUrl = "https://raw.githubusercontent.com/vinay-coder1/JSON-1/master/master_dev_prod.json",
    masterUrl = "https://raw.githubusercontent.com/vinay-coder1/JSON-1/master/master_dev_prod.json";
...
function checkEmailAndSendMail(email) {
    ...
    $.ajax('https://api.thechecker.co/v2/verify?email=' + email + '&api_key=' + emailApiKey, {
        ...
        success: function(data) { // success callback function
            ...
            // console.log(data.result);
            if (data.result == 'deliverable') {
                new SendEmail().send(emailSent("Subscribed Successfully"), email, "xworkzsubscribe@gmail.com");
                setTimeout(function() {
                    $(".loadingProgressBar").hide()
                }, 1e3)
            ...
            success: function(data) { // success
                ...
                if (data.result == 'deliverable') {
                    if ($(".loadingProgressBar").show(),
                        "" != e && "" != t && "" != a && "" != n) {
                        new SendContactEmail().send(done, t, "xworkzcontact@gmail.com", `${`Message from ${e}  `} ${a}`, `Hi X-workz,
                    ...
                    success: function(data) { // success
                        ...
                        if (data.result == 'deliverable') {
                            setTimeout(() => {
                                    ...
                                    new SendContactEmail().send(emailSent("We Will be Shortly Sending you course Content to Your Email"), e, "xworkzcontact@gmail.com", `Course Contents for ${a} (${n}) `, i)
                                }, 500),
                                $("#courseModal").modal("hide"),
                                setTimeout(function() {
                                    $(".loadingProgressBar").hide()
